function homo_out = rsfc_homo_schaefer(scale, subj_ls, HCPA_dir, outname)

% homo_out = rsfc_homo_schaefer(scale, subj_ls, HCPA_dir, outname)
% 
...
for i = 1:length(subjects)
    ...
    for j = 1:length(runs)
        if(~isempty(runs{j}))
            ...
            cd(runs{j})
            fname = [runs{j} '_Atlas_MSMAll_hp0_clean.dtseries.nii'];
