...
for i = 1:length(subjects)
    ...
    lh_fname = fullfile(data_dir, ['lh.' s '_ses01_postproc.nii.gz']);
    rh_fname = fullfile(data_dir, ['rh.' s '_ses01_postproc.nii.gz']);
    mri = MRIread(lh_fname);
    lh_ts = reshape(mri.vol, size(mri.vol,1)*size(mri.vol,2)*size(mri.vol,3), size(mri.vol,4));
    mri = MRIread(rh_fname);
    rh_ts = reshape(mri.vol, size(mri.vol,1)*size(mri.vol,2)*size(mri.vol,3), size(mri.vol,4));
