function homo_out = rsfc_homo_schaefer(scale, AA_ls, WA_ls, outname)
...
filename{1} = CBIG_text2cell(AA_ls);
filename{2} = CBIG_text2cell(WA_ls);
homo_out = zeros(length(filename{1}), 2);
for grp = 1:2
    fprintf('----- Group: %d -----\n', grp)
    num_subs = length(filename{grp});
    for k = 1:num_subs
        fprintf('%dth subject\n', k)
        count = 0;
        filename{grp}{k}
        curr_filename = textscan(filename{grp}{k}, '%s');
                ...
                if(~isempty(curr_filename{i}))
                    ...
                    for c = 1:max(labels)      
                        ...
                        if(~isempty(idx_zerostd))
                            warning('%dth subject in group %d, run %d: %d vertices with label %d have a constant signal.\n', ...
                                k, grp, i, length(idx_zerostd), c)
                            a(:,idx_zerostd) = [];
                    ...
                    homo_out(k,grp) =  homo_out(k,grp) + curr_homo;
                    count = count + 1;
                end
            end
        end

        homo_out(k,grp) = homo_out(k,grp) / count;
